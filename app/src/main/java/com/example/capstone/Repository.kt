...
class Repository private constructor(
    ...
    private var authApi: ApiService, // API for authentication and registration
    private var imageApi: ApiService  // API for image analysis
) {
    ...
    // Image Analysis
    suspend fun analyzeImage(image: String): Flow<SkincareResponse> = flow {
        try {
            val request = ProcessImageRequest(image)
            val response = imageApi.processImage(request) // Asynchronous API call

            if (response.isSuccessful) {
                emit(response.body()!!) // Emit the successful result
            } else {
                throw Exception("Error: ${response.message()}") // Handle error
            }
        } catch (exception: Exception) {
            throw exception // Propagate error to Flow
        }
    }.flowOn(Dispatchers.IO)
